...
public abstract class Controller {
	...
	public boolean onTouchh(float x, float y, Actor touchpad) {
		float width = touchpad.getWidth();
		float height = touchpad.getHeight();
		float radiusX = width / 2;
		float radiusY = height / 2;
		float centerX = touchpad.getX() + radiusX;
		float centerY = touchpad.getY() + radiusY;

		float maxX = centerX + radiusX;
		float minX = Math.abs(centerX - radiusX);
		float maxY = centerY + radiusY;
		float minY = Math.abs(centerY - radiusY);

		// Find the degree that the touch event is at
		double degree = (Math.atan2(y - centerY, x - centerX) / (Math.PI / 180));
		degree *= -1;
		degree += 360;
		if (degree > 360) {
			degree -= 360;
	...
	public void onTouchUp(float x, float y, Actor touchpad) {
		float width = touchpad.getWidth();
		float height = touchpad.getHeight();
